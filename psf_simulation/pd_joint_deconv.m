...
if length(channels) < 1
  error('No valid channels found for deconvolution.\n')
end
...
for s=1:size(w_base,1)
    ...
    %Transfer
    %if ch_opt < length(channels)
    %    db_chs(ch_opt + 1).Image = db_chs(ch_opt).Image;
    %end
end
...
function [ res ] = residual_pd_deconv(channels, db_chs, ch, ...
    ...
    for d = 1:(res_iter + 1)
        ...
        %Show current result
        if strcmp( verbose, 'all')
            figure(iterate_fig(ch));
            if d > 1
                subplot(1, res_iter + 1, 1);
            end
            imshow(db_chs(ch).Image - 1);
            title(sprintf('Startup iterate in Ch %d', ch));
            if d > 1
                subplot(1, res_iter + 1, 1 + (d - 1));
                imagesc(x), axis image, colormap gray;
                title(sprintf('Detail layer %d in Ch %d with lambda_res %5.5g', d - 1, ch, w_res_curr));
            end
            pause(0.1);
        end
        %pause
...
function [ res ] = pd_channel_deconv(channels, ch, x_0 , db_chs, ...
    ...
    %Display it.
    if strcmp(verbose, 'all') % && mod(i,10) == 0
        figure(iterate_fig);
        clf;
        imagesc(f1), colormap gray, axis image, title(sprintf('Local PD iterate %d',0));
        pause(0.1)
    end
...
if lambda_tv > eps
    ...
    fx = imconv(f, fliplr(flipud(dxf)), 'full');
    fx = (lambda_tv * 0.5) * fx(:, 2:end);

    fy = imconv(f, fliplr(flipud(dyf)), 'full');
    fy = (lambda_tv * 0.5) * fy(2:end, :);

    sd_w = 0.15; %weight for second derivatives
    fxx = imconv(f, fliplr(flipud(dxxf)), 'full');
    fxx = (lambda_tv * sd_w) *  fxx(:, 3:end);

    fyy = imconv(f, fliplr(flipud(dyyf)), 'full');
    fyy = (lambda_tv * sd_w) * fyy(3:end, :);

    fxy = imconv(f, fliplr(flipud(dxyf)), 'full');
