...
def to_gray(img: np.ndarray) -> np.ndarray:
    """
    Convert an image array to grayscale float32.
    Supports (H,W), (H,W,3), (H,W,4).
    """
    if img.ndim == 2:
    ...
    if img.ndim == 3 and img.shape[2] == 3:
        # Standard luminance conversion (works well for most cameras)
        r = img[..., 0]
        g = img[..., 1]
        b = img[..., 2]
...
def loadData(show_im=True):
    ...
    # Convert BOTH to grayscale (so everything becomes 2D)
    psf = to_gray(psf)
    data = to_gray(data)

    """In the picamera / Starvis sensor, there is a non-trivial background 
    ...
    This is a simple box-filter downsample used in the original tutorial."""
    ...
    """Normalize power"""
    psf /= (np.linalg.norm(psf.ravel()) + 1e-12)
    data /= (np.linalg.norm(data.ravel()) + 1e-12)
...
def Psi(v):
    return np.stack(
        (np.roll(v, 1, axis=0) - v, np.roll(v, 1, axis=1) - v),
        axis=2
    )
...
def M(vk, H_fft):
    return np.real(
        fft.fftshift(
            fft.ifft2(fft.fft2(fft.ifftshift(vk)) * H_fft)
        )
    )
...
def CT(b):
    ...
    return np.pad(b, ((v_pad, v_pad), (h_pad, h_pad)),
...
def MT(x, H_fft):
    ...
    return np.real(
        fft.fftshift(
            fft.ifft2(fft.fft2(x_zeroed) * np.conj(H_fft))
        )
    )
